...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SpiInit(void)
{
	SCK_PIN = PA5;
	MISO_PIN = PA6;
	MOSI_PIN = PA7;
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SpiMode(byte config)
{
...
*OUTPUT       : none
****************************************************************/
void IN_ELECHOUSE_CC1120::GDO_Set(void)
{
	pinMode(GDO0, INPUT);
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::Reset(void)
{
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::Init(void)
{
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::Init(byte f)
{
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SpiWriteReg(uint16_t addr, byte value)
{
	byte  temp;
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SpiWriteBurstReg(byte addr, byte *buffer, byte num)
{
	byte i, temp;
	byte arrayt[20];
...
*OUTPUT       :none
****************************************************************/
byte IN_ELECHOUSE_CC1120::SpiStrobe(byte strobe)
{
	byte sts;
	digitalWrite(SS_PIN, LOW);
	while (digitalRead(MISO_PIN));
...
****************************************************************/
byte IN_ELECHOUSE_CC1120::SpiReadReg(uint16_t addr)
{
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SpiReadBurstReg(uint16_t addr, byte *buffer, byte num)
{
	byte value, temp, i;
	uint8_t tempExt = (uint8_t)(addr >> 8);
	uint8_t tempAddr = (uint8_t)(addr & 0x00FF);
...
byte IN_ELECHOUSE_CC1120::SpiReadStatus(uint16_t addr)
{
	byte value, temp;
	uint8_t tempExt = (uint8_t)(addr >> 8);
	uint8_t tempAddr = (uint8_t)(addr & 0x00FF);
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::RegConfigSettings(byte ch) 
{ 
	//
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SendData(byte *txBuffer,byte size)
{
...
void IN_ELECHOUSE_CC1120::EnableLNA(void)
{
	digitalWrite(RE_PIN, LOW);
	digitalWrite(TE_PIN, HIGH);
}

void IN_ELECHOUSE_CC1120::DisableLNA(void)
{
	digitalWrite(RE_PIN, LOW);
...
*OUTPUT       :none
****************************************************************/
void IN_ELECHOUSE_CC1120::SetReceive(void)
{	
	byte sts, size;
	byte temp;
	byte buffer[50];
...
****************************************************************/
byte IN_ELECHOUSE_CC1120::CheckReceiveFlag(void)
{  
    byte sts;
	byte reserved;
...
****************************************************************/
byte IN_ELECHOUSE_CC1120::ReceiveData(byte *rxBuffer)
{
	Serial.println();
	byte size;
	byte status[2];
...
void IN_ELECHOUSE_CC1120::ManualCalibration(void) {
...
void IN_ELECHOUSE_CC1120::SetChannel(int ch) {
...
void IN_ELECHOUSE_CC1120::PrintChannel() {
...
int IN_ELECHOUSE_CC1120::Read8BitRssi(void){
...
void IN_ELECHOUSE_CC1120::setThisAddress(uint8_t address)
{
   	 e_thisAddress = address;
}
void IN_ELECHOUSE_CC1120::setHeaderFrom(uint8_t from)
{
   	 e_txHeaderFrom = from;
}
void IN_ELECHOUSE_CC1120::setHeaderTo(uint8_t to)
{
   	 e_txHeaderTo = to;
}
void IN_ELECHOUSE_CC1120::setHeaderMaster(uint8_t master)
{
   	 e_txHeaderMaster = master;
}
void IN_ELECHOUSE_CC1120::setHeaderType(uint8_t type)
{
   	 e_txHeaderType = type;
}
 ...
 uint8_t  IN_ELECHOUSE_CC1120::headerFrom( )
{
 ...
 uint8_t  IN_ELECHOUSE_CC1120::headerMaster( )
{ 
	return  e_rxHeaderMaster;
}
 uint8_t  IN_ELECHOUSE_CC1120::headerType( )
{
 	return  e_rxHeaderType;
  }


byte IN_ELECHOUSE_CC1120::setSyncWords(const uint8_t* syncWords, uint8_t len)
{
    if (!syncWords || len != 4)
...
void IN_ELECHOUSE_CC1120::setTxPowerTo5mW( )
{
...
uint16_t IN_ELECHOUSE_CC1120::culCalcCRC(uint8_t crcData, uint16_t crcReg) {
 uint8_t i;
 for (i = 0; i < 8; i++) {
...
uint16_t  IN_ELECHOUSE_CC1120::CRC(uint8_t *txBuffer,uint8_t size ) {
	uint16_t checksum;
	uint8_t i;
...
signed char IN_ELECHOUSE_CC1120::rssi_dbm(unsigned char raw_rssi)
{
  int16_t dbm = 0;
