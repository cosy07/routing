...
void Datagram::setHeaderSource(uint8_t source)
{
	_driver.setHeaderSource(source);
}

void Datagram::setHeaderDestination(uint8_t destination)
{
...
uint8_t Datagram::headerSource()
{
	return(_driver.headerSource());
}
uint8_t Datagram::headerDestination()
{
...
uint8_t Datagram::getThisAddress()
{
	return _thisAddress;
}
...
void Datagram::addRouteTo(uint8_t  dest, uint8_t  next_hop, uint8_t state, uint8_t hop, unsigned long lastTime)
{
	uint8_t i;
...
////////////////////////////////////////////////////////////////////
Datagram::RoutingTableEntry* Datagram::getRouteTo(uint8_t  dest)
{
	uint8_t i;
	for (i = 0; i < ROUTING_TABLE_SIZE; i++)
...
////////////////////////////////////////////////////////////////////
bool Datagram::deleteRouteTo(uint8_t dest)
{
	uint8_t i;
	for (i = 0; i < ROUTING_TABLE_SIZE; i++)
	{
...
////////////////////////////////////////////////////////////////////
void Datagram::checkRoutingTable()
{
		...
		while (millis() - startTime < TIME_TERM)
		{
			...
			if (available())
			{
				if (recvData(temp_buf) &&  _rxHeaderFrom == to)
				{
					...
					if (type == CHECK_ROUTING && _rxHeaderType == CHECK_ROUTING)
						return true;
				}
			}
		}
...
void Datagram::getPath(uint8_t address, uint8_t* path)
{
...
*OUTPUT       : 마스터 번호
****************************************************************/
/*uint16_t Datagram::convertToMasterNumber(uint16_t address)
{
	uint16_t masterNum;
...
****************************************************************/
uint8_t Datagram::G_get_i_row_node_list(uint8_t row_number, uint8_t *node_list)
{
	uint8_t j = 0;
...
void Datagram::FromGatewayToMaster() {
	...
	while (1)
	{
		for (int i = 1; i <= master_num; i++)
		{
			...
			if (!checkReceive[i])
				continue;
			if (!sendToWaitAck(_thisAddress, getRouteTo(address_i)->next_hop, _thisAddress, address_i, CHECK_ROUTING, NONE, NONE, NONE, NONE, temp_buf, sizeof(temp_buf)), 2000)
			{
...
****************************************************************/
int8_t Datagram::G_find_2ndRow_master(uint8_t* unknown_node_list)
{
	Serial.println("[row2]");
...
*OUTPUT       :
****************************************************************/
int8_t Datagram::G_find_multihop_node(uint8_t number_of_unknown_node, uint8_t* unknown_node_list)
{
...
*OUTPUT       :  성공 여부
****************************************************************/
bool Datagram::G_request_path_one_by_one(uint8_t unknown_address, uint8_t row_number, uint8_t* node_list, byte number_of_node, uint8_t type)
{
	//_driver.Init(ch);
...
*OUTPUT       :
****************************************************************/
bool Datagram::G_discoverNewPath(uint8_t address, uint8_t row_number)//, bool child)
{
...
void Datagram::changeNextHop(uint8_t address)
{
	for (int i = 1; i <= master_num; i++)
	{
		if (parentMaster[i] == address)
		{
			addRouteTo(i, getRouteTo(address)->next_hop, Valid, getRouteTo(address)->hop + 1);
			changeNextHop(i);
		}
	}
}
...
*OUTPUT       :
****************************************************************/
void Datagram::G_find_error_node(uint8_t address)
{
...
void Datagram::FromMasterToGateway() {
	...
	while (1)
	{
		if (check_table_time + 600000 < millis())
		{
			...
			Serial.print(check_table_time + 60000);
			Serial.print(" ");
...
void Datagram::newMaster()
{
	...
	while (!sendToWaitAck(_thisAddress, candidate_parent, _thisAddress, candidate_parent, NEW_NODE_REGISTER, NONE, NONE, NONE, NONE, temp_buf, sizeof(temp_buf)))
	{
		while (millis() - waitTime < 10000)
		{
			SetReceive();
